...
/*   Updated: 2020/02/13 15:27:35 by scleerdi      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static void	place_tail(t_stacks *ab, int tail, int loc_t)
{
...
static int	locate_tail(t_stacks const *ab, int tail)
{
	int			i;
	t_stacks	dup_ab;
	t_stacks	hold_ab;
...
int			push_lowest_value_to_b(t_stacks *ab)
{
	...
	tail = determine_tail(ab);
	ab->lead_b = tail;
	loc_t = locate_tail(ab, tail);
	place_tail(ab, tail, loc_t);
	return (tail);
}
